...
export class SignalingService {
		...
		this.ws.onmessage = event => {
			try {
				...
				if (
					...
					const payloadRecord = envelope.payload as { token?: string };
	...
	private sendRaw(envelope: AnyEnvelope): void {
	...
	public disconnect(): void {
		...
		if (this.currentRoom && this.ws && this.ws.readyState === WebSocket.OPEN) {
			this.sendRaw({ v: 1, type: MessageType.LEAVE, room: this.currentRoom });
		}
